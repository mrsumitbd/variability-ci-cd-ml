package org.crashub.bash.script;
...
public class Script {
	...
	static Expression.VarRef _VAR_REF(Tree tree) {
		...
		case java_libbashParser.USE_ALTERNATE_WHEN_UNSET:
			return new Expression.VarRef.Default(child.getChild(0).getText(), child.getType(),
	...
	private Node _COMMAND(Tree tree) {
		...
		case java_libbashParser.STRING: {
			if (child.getChild(0).getType() == java_libbashParser.LOCAL) {
				...
				String identifier = assertTree(t.getChild(0), java_libbashParser.NAME, java_libbashParser.LETTER)
						.getText();
	...
	public void getCommandList(Tree tree) {
		if (tree.getType() == java_libbashParser.COMMAND) {
			final Tree child = tree.getChild(0);

			if (child.getType() == java_libbashParser.STRING) {
				if (child.getChildCount() > 0) {
					StringBuilder strb = new StringBuilder();

					for (int index = 0; index < child.getChildCount(); index++) {
						strb.append(child.getChild(index).getText());
					}
				...
				for (int i = 0; i < tree.getChildCount(); i++) {
					getCommandList(tree.getChild(i));
				}
			}
		} else {
			for (int i = 0; i < tree.getChildCount(); i++) {
	...
	public void getCommandListWithCompundCmd(Tree tree, Map<String, String> envmap) {
		if (tree.getType() == java_libbashParser.COMMAND) {
			int childcount = tree.getChildCount();

			for (int childindex = 0; childindex < childcount; childindex++) {

				final Tree child = tree.getChild(childindex);

				if (isCompoundCmd(child) || isCmdflag(child)) {
					getCascadedCmd(tree, childindex + 1);
					continue;
				...
				if (child.getType() == java_libbashParser.STRING) {
					if (child.getChildCount() > 0) {
						...
						for (int index = 0; index < child.getChildCount(); index++) {
							if (child.getChild(index).getType() == java_libbashParser.VAR_REF) {
								String envname = exapndNodeAsText(child.getChild(index));
								String envcmd = "";
								if (envmap.containsKey(envname)) {
									envcmd = envmap.get(envname);
									if(isCompoundCmd(envcmd))
									{
										continue;
									}
								} else {
									envcmd = envname.toLowerCase();
									if(isCompoundCmd(envcmd))
									{
										continue;
									}
								...
								strb.append(envcmd);
							} else if (child.getChild(index).getType() == java_libbashParser.DOUBLE_QUOTED_STRING) {
								String envname = exapndNodeAsText(child.getChild(index));
								String envcmd = "";
								if (envmap.containsKey(envname)) {
									envcmd = envmap.get(envname);
								...
								if (isUserScript(strb.toString())) {
									Tree parent=child.getParent();
									if(parent.getChildCount()>index)
									{
										String param=exapndNodeAsText(parent.getChild(index+1));
										
										if(param.toLowerCase().contains("test"))
										{
											strb.append("_"+param);
										}
									}
						...
						if (strb.toString().equals("DOUBLE_QUOTED_STRING")) {
							commands.add("EMBEDED_BUILD_CODE");
						} else {
							commands.add(strb.toString());
						}
						break;
					...
					for (int i = 0; i < tree.getChildCount(); i++) {
						getCommandListWithCompundCmd(tree.getChild(i), envmap);
					}
				}
			}
		} else {
	...
	public boolean isUserScript(String str) {
		boolean flag = false;
		if (str.endsWith(".sh") || str.endsWith(".py") || str.startsWith("./") || str.equals("make")) {
			flag = true;
		}
	...
	public void getEnvDefination(Tree tree, Map<String, String> envdef) {
		if (tree.getType() == java_libbashParser.VARIABLE_DEFINITIONS) {
			final Tree child = tree.getChild(0);
			...
			if (child.getType() == java_libbashParser.EQUALS) {
				...
				if (child.getChildCount() > 1) {
					Tree left = child.getChild(0);
					Tree right = child.getChild(1);

					if (left.getType() == java_libbashParser.NAME) {
						key = left.getText();
						keyfound = true;
					}
					if (right != null) {
						StringBuilder strb = new StringBuilder();
						for (int rindex = 0; rindex < right.getChildCount(); rindex++) {
							Tree rightchild = right.getChild(rindex);

							if (rightchild.getType() == java_libbashParser.DOUBLE_QUOTED_STRING) {
								String nodetext = exapndNodeAsText(rightchild);
								nodetext = nodetext.replaceAll("\"", "");
								strb.append(nodetext);
							} else if (rightchild.getType() == java_libbashParser.SINGLE_QUOTED_STRING) {
								String nodetext = exapndNodeAsText(rightchild);
								nodetext = nodetext.replaceAll("'", "");
								strb.append(nodetext);
							} else {
								strb.append(rightchild.getText());
							}
						}

						value = strb.toString();
						valuefound = true;
//							if (rightchild != null && rightchild.getType() == java_libbashParser.NAME) {
...
//							}
					...
					if (keyfound == true && valuefound == true) {
						envdef.put(key, value);
					}
				}
			} else {
				for (int i = 0; i < tree.getChildCount(); i++) {
					getEnvDefination(tree.getChild(i), envdef);
				}
			}
		} else {
			for (int i = 0; i < tree.getChildCount(); i++) {
	...
	private String exapndNodeAsText(Tree node) {
		StringBuilder strb = new StringBuilder();

		if (node != null) {
			for (int index = 0; index < node.getChildCount(); index++) {
				Tree child = node.getChild(index);
				strb.append(child.getText());
			}
		}
	...
	public boolean isCompoundCmd(Tree node) {
		boolean flag = false;

		final Tree child = node.getChild(0);
		StringBuilder text = new StringBuilder();
		for (int index = 0; index < node.getChildCount(); index++) {
			text.append(node.getChild(index).getText());
		}

		List<String> cmdList = Arrays.asList(Config.compoundcmds);

		for (String cmd : cmdList) {

			if (text.toString().equals(cmd)) {
				flag = true;
				break;
			}
		}
	...
	public boolean isCmdflag(Tree node) {
		boolean flag = false;

		final Tree child = node.getChild(0);

		if (child!=null && child.getType() == java_libbashParser.MINUS) {
			flag = true;
		}
	...
	public String getCascadedCmd(Tree node, int childindex) {
		String cmd = "test";
	...
	public boolean isCompoundCmd(String treecmd) {
		boolean flag = false;
		...
		for (String cmd : cmdList) {
			...
			if (treecmd.toString().equals(cmd)) {
				flag = true;
				break;
			}
		}
